...
def on_text(text):
    ...
    if "_" not in mot.text and mot.text!=text_initial:
        essais.append(mot.text)
        coups.text=str(len(essais))
        essai=mot.text.lower()
        if essai<solution:
            if apres.text.lower()<essai:
                apres.text=mot.text
            mot.text="_"*len(solution)
        elif essai>solution:
            if essai<avant.text.lower() or avant.text=="":
                avant.text=mot.text
            mot.text="_"*len(solution)
        elif mot.text.lower()!=solution.lower():
            message.text=u"presque %s"%mot.text
            mot.text="_"*len(solution)
        else:
            message.text="Bravo !!!"
            pause.append("fin")
            #pyglet.clock.unschedule(sprites_update)
